    ...
    return (
        <Card className={classes.card}>
            ...
            <div className={classes.details}>
                {post?.tags && post?.tags.length > 0 && post?.tags[0] !== '' && <Typography variant="body2" color="textSecondary">{post.tags.map((tag) => `#${tag} `)}</Typography>}
            </div>
            {post.title && <Typography className={classes.title} variant="h5" gutterBottom>{post.title}</Typography>}
            <CardContent>
                {post.message && <Typography variant="body2" color="textSecondary" component="p">{post.message}</Typography>}
            </CardContent>
